...
func Probe(ctx context.Context, path string) (TrackMetadata, error) {
	...
	cmd := exec.CommandContext(ctx, "ffprobe", "-v", "error", "-show_entries", "format=duration,bit_rate:format_tags:stream=codec_type,codec_name,sample_rate,channels,bit_rate:stream_tags", "-of", "json", path)
