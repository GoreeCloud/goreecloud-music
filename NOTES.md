...
- Schema version 2 introduced explicit `library_memberships` for per-profile library authorization; schema version 3 added durable `library_files` filesystem observations for incremental scan reconciliation; schema version 4 adds durable `library_file_metadata` bound to scanner observations.
- The current Development application-state backend is SQLite through Go `database/sql` and `modernc.org/sqlite`.
- Schema-v1 library owners are preserved during migration to v2 by transactionally materializing owner memberships before the stored schema version advances; the v2→v3 migration adds scanner state without changing those authorization facts; the v3→v4 migration adds metadata state while preserving existing library/file observations.
...
- Bounded embedded metadata extraction is implemented for MP3 ID3v2.3/v2.4 and FLAC Vorbis Comments. Extracted title, artist, album, album artist, genre, date/year text, track-number text, and disc-number text are persisted per scanner file observation without creating canonical Recording/Release identity.
- Metadata extraction requires library edit authorization and metadata retrieval requires library read authorization. Source reads are rooted with Go `os.Root`, symbolic-link components remain rejected, and scanner size/mtime is revalidated both before parsing and immediately before persistence.
- Cross-profile tests verify default denial, explicit grants, independent profile state, authorization-revocation suppression, and the bounded Recently Added and metadata access paths implemented through PR #16.
...
- PR #16 merged the bounded embedded metadata extraction foundation to signed `main` as `bc90686a18338afef650b73357f454d1be541afb`; exact candidate `fc9613d94493e2d5c355f3aba7b6bd2a24a2d334` passed CI `35030658032` and Platform Contract `35030658657`, and post-merge `main` passed CI `35030857299` and Platform Contract `35030858110`.

## Open architecture decisions and implementation obligations

- Complete the remainder of Milestone 1: approved sidecar metadata extraction, artwork handling, additional embedded metadata formats where required, canonical Recording/Release/Source Item/Playable Asset ingestion from scanned files, codec/container probing, filesystem event watchers/change notifications, complete multi-user isolation across remaining library paths, library/search/profile-state/metadata APIs, and remaining persistent first-party state.
...
- Embedded metadata is a per-file observation layer, not canonical Recording/Release identity. Canonical ingestion and match validation remain separate pending work.
- Complete production persistence qualification, including concurrency/load evidence, corruption handling, migration/export behavior, and Everkeep-aligned backup/restore acceptance. The merged SQLite Development backend, scanner state, metadata state, profile-state operations, and Recently Added query are not production/recovery acceptance.
