...
- Bounded read-only embedded metadata extraction is implemented for MP3 ID3v2.3/v2.4 and FLAC Vorbis comments. The metadata layer normalizes title, artist, album, album artist, genre, date/year, track number, and disc number while explicitly stopping before canonical Recording/Release/Source Item/Playable Asset ingestion.
...
- Complete the remainder of Milestone 1: approved sidecar metadata extraction, embedded-tag/container formats beyond the current MP3/FLAC subset, artwork handling, canonical Recording/Release/Source Item/Playable Asset ingestion from scanned files, codec/container probing, filesystem event watchers/change notifications, complete multi-user isolation across remaining library paths, library/search/profile-state APIs, and remaining persistent first-party state.
